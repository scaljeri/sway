...
(function (ns) {
     ...
     * </ul>
     * Between two tables only one relation can be defined!
     *
     * Furthermore, a Field can perform data validation and transformation
     *
     ...
     * And, if an 'Account' and 'Supplier' both <tt>has\_one</tt> 'AccountHistory', it could look like this
     *
     ...
     *           new Sway.data.Relation({ key: 'accountHistory', type: 'has_one', through: 'Account', model: 'AccountHistory' })
     *           ....
     *      ]) ;
     *
     ...
     *      ...
     *
     * And the initialization process goes as follows:
     *
     ...
     * This association defines a one-to-many or a many-to-many relationship.
     *
     ...
     *      @param {String}  [options.model] referenced model (only for associations)
     *      @param {String}  [options.through] specifies a join model. Only available for <tt>HAS\_ONE</tt> and <tt>HAS\_MANY</tt> associations (only for associations)
     *      @param {String}  [options.friendlyName] description of the field
     *      @param {Boolean} [options.PK=false] primary key field (there can only be one primary key field). If not defined, one will be created.
     ...
     *      @param {Array}   [options.transformers] list of transformer objects. TODO
     *      @param {Array}   [options.validators] list of validation functions. TODO
     */
    var Field = function (key, options) {
        ...
        switch (this.type) {
            ...
            case 'has_and_belongs_to_many':
                ...
                this.defaultValue = options.defaultValue||[] ;
                break;
            default: // its not an association
                if ( this.model || this.through ) { // check input
